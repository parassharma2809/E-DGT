...
def train(opt, dset, model, criterion, optimizer, epoch, previous_best_acc):
    ...
    dset.set_mode("train")
    model.train()
    train_loader = DataLoader(dset, batch_size=opt.bsz, shuffle=True, collate_fn=lambda b: pad_collate(b, opt))
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader)):
        # model_inputs, targets, _ = preprocess_inputs(batch, opt.max_transcript_l, opt.max_vid_l, device=opt.device)
        model_inputs, targets, _ = preprocess_inputs(batch, opt.max_transcript_l, opt.max_vid_l, device='cpu')
        print(f'batch idx {batch_idx} and log freq{opt.log_freq}')
        ...
        if batch_idx % opt.log_freq == 0:
            ...
            results_dict['Train/Acc'] = (train_acc, niter)
            results_dict['Train/Loss'] = (train_loss, niter)
            ...
            results_dict['Valid/Loss'] = (valid_loss, niter)
...
def validate(opt, dset, model, mode="valid"):
    ...
    model.eval()
    valid_loader = DataLoader(dset, batch_size=opt.test_bsz, shuffle=False, collate_fn=lambda b: pad_collate(b, opt))
    ...
    for _, batch in enumerate(valid_loader):
        ...
        model_inputs, targets, qids = preprocess_inputs(batch, opt.max_transcript_l, opt.max_vid_l, device='cpu')
        outputs = model(*model_inputs)
        loss = criterion(outputs, targets)
...
if __name__ == "__main__":
    ...
    import sys
    sys.argv[1:] = ["--input_streams", "transcript", "audio", "--max_transcript_l", "500", "--debug"]
    ...
    if torch.cuda.is_available():
        ...
        # Depends on number of GPUs we want to split the model on. This can be changed
        # if opt.device == 0:
        #     # Using GPU
        #     # This will store and run parallely on multiple GPUs.
    ...
    for epoch in range(opt.n_epoch):
        ...
        if opt.debug:
            break
    with open(os.path.join(opt.results_dir, "all_scalars_result.json"), 'w') as f:
